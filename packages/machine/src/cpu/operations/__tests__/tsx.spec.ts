...
/**
 * Operation semantics: TSX (Transfer Stack Pointer to X)
 * - X = SP.
 * - Sets Z if X==0, N from bit7 of X.
 */
describe(tsx.name, () => {
	describe.each`
		stackPointer | expectedX | expectedNegative | expectedZero
		${0x00}      | ${0x00}   | ${false}         | ${true}
		${0x01}      | ${0x01}   | ${false}         | ${false}
		${0xff}      | ${0xff}   | ${true}          | ${false}
	`(
		'when stack pointer is $stackPointer',
		({ expectedNegative, expectedX, expectedZero, stackPointer }) => {
			it(
